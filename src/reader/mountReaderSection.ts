...
function renderDetailCard(
  ...
  if (paper.status !== "resolved") {
    if (!paper.referenceText) return "";
    return `<aside class="rfz-detail-card rfz-reference-detail" data-detail-card="" data-paper-id="${escapeAttribute(paper.id)}">
      <button type="button" class="rfz-card-close" data-detail-close="" aria-label="Close Reference details">×</button>
      <strong class="rfz-card-title" data-translation-text="">${escapeHTML(paper.title)}</strong>
      <section class="rfz-reference-text"><strong>Reference</strong><p data-reference-text="" data-translation-text="">${renderReferenceWithLinks(paper.referenceText)}</p></section>
    </aside>`;
  }
  const badges = [
...
const READER_STYLES = `
  ...
  .rfz-paper--resolved .rfz-paper-title--has-abstract { font-style: italic; text-decoration-line: underline; text-decoration-color: var(--rfz-accent); text-underline-offset: 2px; }
  ...
  .rfz-reference-text { margin-top: 14px; font-size: 13px; line-height: 1.55; }
  .rfz-reference-text > strong { display: block; margin-bottom: 5px; font-size: 12px; }
  .rfz-reference-text p { margin: 0; white-space: pre-wrap; overflow-wrap: anywhere; }
