...
- 更新 Reference 查询缓存版本，升级后重新解析和检索参考文献，避免继续显示旧缓存中的错误题名。

### 修复

- 修复 MinerU 参考文献中多个中间名缩写、全名与缩写混排及姓名异常断词导致作者被当作题名，或显示 `Title unavailable` 的问题。
- 修复混合姓名格式下遗漏作者检索信息的问题，兼容不带句点的中间名缩写和紧邻姓氏的连续缩写。

### 工程

- 增加真实 MinerU 参考文献、姓名格式及 Reader 检索与显示的回归测试；验证包含 111 条参考文献的语料中 7 条错误题名已修正，其余 104 条题名保持不变。
- 将本地 CodeGraph 索引目录 `.codegraph/` 加入 Git 忽略规则。
